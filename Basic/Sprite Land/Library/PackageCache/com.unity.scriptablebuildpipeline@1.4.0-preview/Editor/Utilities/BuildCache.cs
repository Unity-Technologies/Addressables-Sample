...
namespace UnityEditor.Build.Pipeline.Utilities
{
    ...
    public class BuildCache : IBuildCache, IDisposable
    {
        ...
        static string FormatBytes(long bytes)
        {
            ...
            return string.Format("{0:0.##} {1}", dbytes, suffix[i]);
        }
        ...
        struct CacheFolder
        {
            ...
            public DateTime LastAccessTimeUtc { get { return directory.LastAccessTimeUtc; } }
        }
        ...
        public static void PruneCache(bool prompt)
        {
            ...
            long pruneSize = maximumSize * 1073741824L; // gigabytes to bytes
            long currentSize = 0L;
            ...
            foreach (var subDirectory in subDirectories)
            {
                ...
                currentSize += cacheFolder.Length;
            }

            if (currentSize < pruneSize)
            {
                if (prompt)
                    Debug.LogFormat("Current build cache size {0}, prune threshold {1} GB. No prune performed. You can change this value in the \"Edit/Preferences...\" window.", FormatBytes(currentSize), maximumSize);
                return;
            }

            if (prompt)
            {
                if (!EditorUtility.DisplayDialog("Prune Build Cache", string.Format("Current build cache size is {0}, which is over the prune threshold of {1}. Do you want to prune your build cache now?", FormatBytes(currentSize), FormatBytes(pruneSize)), "Yes", "No"))
                    return;
