...
namespace UnityEngine.ResourceManagement
{
    ...
    public class ResourceManager : IDisposable
    {
        ...
        internal int OpCacheCount { get { return m_OpCache.Count; } }
        ...
        Dictionary<int, IAsyncOperation> m_OpCache = new Dictionary<int, IAsyncOperation>();
        Action<AsyncOperationHandle, ResourceManager.DiagnosticEventType, int, object> m_diagnosticsHandler;
        ...
        private AsyncOperationHandle ProvideResource(IResourceLocation location, Type desiredType = null)
        {
            ...
            if (m_OpCache.TryGetValue(hash, out op))
            {
            ...
            op = CreateOperation<IAsyncOperation>(provType, provType.GetHashCode(), hash);

            // Calculate the hash of the dependencies
            int depHash = CalculateLocationsHash(location.Dependencies);
        ...
        void OnOperationDestroyCached(IAsyncOperation o)
        {
            ...
            if(cachable != null)
                m_OpCache.Remove(cachable.Hash);
        }

        internal T CreateOperation<T>() where T : IAsyncOperation
        {
            var actualType = typeof(T);
            return CreateOperation<T>(actualType, actualType.GetHashCode());
        }

        internal T CreateOperation<T>(Type actualType, int typeHash) where T : IAsyncOperation
        {
            var op = (T)Allocator.New(actualType, typeHash);
            op.OnDestroy = m_ReleaseOpNonCached;
            return op;
        }

        internal T CreateOperation<T>(Type actualType, int typeHash, int operationHash) where T : IAsyncOperation
        {
            if (operationHash == 0)
                return CreateOperation<T>(actualType, typeHash);

            var op = (T)Allocator.New(actualType, typeHash);
            op.OnDestroy = m_ReleaseOpCached;
            var cachable = op as ICachable;
            if (cachable != null)
                cachable.Hash = operationHash;
            m_OpCache.Add(operationHash, op);
            return op;
        }
        ...
        {
            var cop = CreateOperation<CompletedOperation<TObject>>();
        ...
        private GroupOperation AcquireGroupOpFromCache(int hash)
        {
            ...
            if (m_OpCache.TryGetValue(hash, out opGeneric))
            {
        ...
        internal AsyncOperationHandle<IList<AsyncOperationHandle>> ProvideResourceGroupCached(IList<IResourceLocation> locations, int groupHash, Type desiredType, Action<AsyncOperationHandle> callback)
        {
            ...
            if (op == null)
            {
                op = CreateOperation<GroupOperation>(typeof(GroupOperation), s_GroupOperationTypeHash, groupHash);
        ...
        {
            var op = CreateOperation<ChainOperation<TObject, TObjectDependency>>();
        ...
        class InstanceOperation : AsyncOperationBase<GameObject>
        {
            ...
            public void Init(IInstanceProvider instanceProvider, InstantiationParameters instantiationParams, AsyncOperationHandle<GameObject> dependency)
            {
            ...
            protected override string DebugName { get { return string.Format("Instance<{0}>({1}", m_instanceProvider.GetType().Name, m_dependency.DebugName); } }

            protected override void Destroy()
            {
                m_dependency.Release();
                if(m_instance != null)
                    m_instanceProvider.ReleaseInstance(m_instance);
            }
            ...
            protected override void Execute()
            {
                ...
                {
                    m_instance = m_instanceProvider.ProvideInstance(m_dependency, m_instantiationParams);
        ...
        public AsyncOperationHandle<GameObject> ProvideInstance(IInstanceProvider provider, IResourceLocation location, InstantiationParameters instantiateParameters)
        {
            ...
            var baseOp = CreateOperation<InstanceOperation>(typeof(InstanceOperation), s_InstanceOperationTypeHash);

            baseOp.Init(provider, instantiateParameters, depOp);
