...
namespace AddressableAssetsIntegrationTests
{
    internal abstract partial class AddressablesIntegrationTests : IPrebuildSetup
    {
        ...
        public IEnumerator ReleaseInstance_WhenObjectIsUnknown_LogsErrorAndDestroys()
        {
            ...
            LogAssert.Expect(LogType.Error, new Regex("Addressables.ReleaseInstance was called on.*"));
            yield return null;
        ...
        // this test should be removed. we should test that catalogs can merge at a lower level
        [UnityTest]
        public IEnumerator LoadAsset_CanLoadFromMergedCatalog() 
        {
            yield return Init();
            string label = AddressablesTestUtility.GetPrefabUniqueLabel("EXTRA", 0);
            AsyncOperationHandle<GameObject> op = m_Addressables.LoadAsset<GameObject>(label);
            yield return op;
            Assert.AreEqual(AsyncOperationStatus.Succeeded, op.Status);
            Assert.IsTrue(op.Result != null);
